# **Mission Briefing: Operation valkyrie_aegis - Day 1**

## **1. Mission Overview**
- **SITREP UPDATE:** 
    - **DAY1**

## **2. Mission Overview**
- **Objective:** 
- **Target:** 
- **Coordinates:** 
- **Time on Target (TOT):** 
- **Weather Conditions:** 
- **Mission Date/Time**

**Mission Commander:** 
**Flight Lead:** 

## **3. Flight Roster**
| **Flight Name**   | **A/C Type** | **Callsign** | **Role** | **Pilot** | **Notes**    |
|---------------|----------|-------------------|------------|------------|--------------|
| Joker 2       | F/A-18C  | Joker 2-1         | STRIKE     | Lobo       | Flight Lead  | 
| Joker 2       | F/A-18C  | Joker 2-2         | STRIKE     | hitchcock  |              |

## **4. Support Assets**
| **Flight Name**   | **A/C Type** | **Callsign** | **Role** | **Notes**      |
|---------------|----------|-------------------|------------|------------------|
| FOCUS 3       | E-2D     | FOCUS 3-1         | AWACS      | NORTH AWACS      |
| DARKSTAR 4    | E-3A     | DARKSTAR 4-1      | AWACS      | SOUTH AWACS      |
| SHELL 2       | KC-130   | SHELL 2-1         | TANKER     | DROGE TANKER     |

## **5. Divert Airfield's**

| **Airfield** | **Freq (MHz)**| **Mode** | **TACAN** | **ICLS/ILS** | **Runway** | **Notes**   |
|--------------|---------------|----------|-----------|--------------|------------|-------------|
| AKROTIRI     | 252.000 Mhz   | AM       | 107X      | 109.70       | RW 28      | NORTH DIVERT|
| RAMAT DAVID  | 251.300 Mhz   | AM       | 84X       | 111.10       | RW 33      | SOUTH DIVERT |
## **6. Target Information:**
- **Info**
### **Satelite Picture**
![Screenshot](https://github.com/hitchc8ck/dcs/blob/main/missions/campaigns/syria/op_valkyrie_aegis/mission%20data/images/air_defence_academy.png)

### **Enemy Threats:**
- **Info**

## **7. Rules of Engagement (ROE)**
- **Weapons Hold** on all enemy air defense and military targets until confirmed.  
- **Avoid collateral damage** to civilian infrastructure.  
- **Minimize exposure** to enemy radar, SAM and AAA

## **8. Mission Phases**
### **Phase 1:**
- **Info**
### **Phase 2:**
- **Info**
### **Phase 3:**
- **Info**
### **Phase 4:**
- **Info**
## **9. Success Criteria**
- **70% of air defenses are destoryed**
- **30% of ground assets are destoryed**